...
class DataProcessing:
    ...
    def get_class_weights(dataset_name):
        ...
        num_per_class = []
        if dataset_name is 'Car':
            num_per_class = np.array([2668392, 117532, 322819, 139757, 1083361],dtype=np.int32)
...
class Plot:
    ...
    def draw_pc_sem_ins(pc_xyz, pc_sem_ins, plot_colors=None):
        ...
        for id, semins in enumerate(sem_ins_labels):
            ...
            sem_ins_bbox.append( [[xmin, ymin, zmin], [xmax, ymax, zmax], [min(tp[0], 1.), min(tp[1], 1.), min(tp[2], 1.)]])
