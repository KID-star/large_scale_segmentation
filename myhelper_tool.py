...
class ConfigS3DIS:
    ...
    num_points_k = [40960, 10240, 2560, 640, 160]
    up_idx = [0, 1, 3, 6, 10, 2, 4, 7, 11, 5, 8, 12, 9, 13, 14]
    ...
    d_in_Ux = [8, 40, 168, 424, 936]  # feature dimension
    d_out_Ux = [32, 128, 256, 512, 1024]  # feature dimension
    en_d_in = [1984, 1792, 1856, 2048, 2336]
    # en_d_in = [1536, 1600, 1792, 2016, 2312]
...
class DataProcessing:
    ...
    @staticmethod
    def data_aug_without_label(xyz, color, idx, num_out):
        num_in = len(xyz)
        dup = np.random.choice(num_in, num_out - num_in)
        xyz_dup = xyz[dup, ...]
        xyz_aug = np.concatenate([xyz, xyz_dup], 0)
        ...
        return xyz_aug, color_aug, idx_aug
    ...
    def get_class_weights(dataset_name):
        ...
        if dataset_name == 'S3DIS':
            num_per_class = np.array([97061545, 6411292, 1685582, 47580250], dtype=np.int32)
        elif dataset_name == 'Car':
            num_per_class = np.array([2668392, 117532, 322819, 139757, 1083361],
                                     dtype=np.int32)
        elif dataset_name == 'every_Car':
            num_per_class = np.array([886147, 72844, 416119, 78639, 14051585],
                                     dtype=np.int32)
